...
min_vals <- as.data.frame(apply(select(froh_data, c("NSEG","froh")), 2, FUN = min, na.rm = TRUE))
colnames(min_vals)[1] <- "min"
mean_vals <- as.data.frame(apply(select(froh_data, c("NSEG","froh")), 2, FUN = mean, na.rm = TRUE))
